...
namespace Abp.Authorization
{
        ...
        where TUser : AbpUser<TUser>
    {
        ...
        [UnitOfWork]
        public virtual async Task<AbpLoginResult<TTenant, TUser>> LoginAsync(string userNameOrEmailAddress, string plainPassword, bool shouldLockout = true)
        {
        ...
        protected virtual async Task<AbpLoginResult<TTenant, TUser>> LoginAsyncInternal(string userNameOrEmailAddress, string plainPassword, bool shouldLockout)
        {
            ...
            using (UnitOfWorkManager.Current.DisableFilter(AbpDataFilters.MayHaveTenant))
            {
                ...
                else if(user != null && user.TenantId != null)
                {
                    tenant = await TenantRepository.FirstOrDefaultAsync(t => t.Id == user.TenantId);
                    if (tenant == null)
                    {
        ...
        protected virtual async Task SaveLoginAttempt(AbpLoginResult<TTenant, TUser> loginResult, string userNameOrEmailAddress)
        {
            using (var uow = UnitOfWorkManager.Begin(TransactionScopeOption.Suppress))
            {
                ...
                using (UnitOfWorkManager.Current.SetTenantId(tenantId))
                {
                    ...
                    {
                        TenantId = tenantId,
                        TenancyName = loginResult?.Tenant?.Name,
