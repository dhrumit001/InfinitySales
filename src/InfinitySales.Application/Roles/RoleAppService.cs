...
namespace InfinitySales.Roles
{
    ...
    public class RoleAppService : AsyncCrudAppService<Role, RoleDto, int, PagedResultRequestDto, CreateRoleDto, RoleDto>, IRoleAppService
    {
        ...
        public async Task<RoleDto> CreateUpdate(RoleDto input)
        {
            if (input.Id > 0)
                return await Update(input);

            var newRole = ObjectMapper.Map<CreateRoleDto>(input);
            return await Create(newRole);
        }
