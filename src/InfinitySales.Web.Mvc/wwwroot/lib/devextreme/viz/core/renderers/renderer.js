...
function adjustLineHeights(items) {
    ...
    for (i = 1, ii = items.length; i < ii; ++i) {
        ...
        if (item.line === currentItem.line) {
            ...
            currentItem.inherits = currentItem.inherits || 0 === _parseFloat(item.height);
            item.height = NaN
        } else {
            currentItem = item
        }
...
function setMaxSize(maxWidth, maxHeight) {
    ...
    var ellipsis, ellipsisWidth, that = this,
        lines = [],
        textChanged = false,
        textIsEmpty = false,
...
SvgElement.prototype = {
    ...
    smartAttr: function(attrs) {
        var that = this;
        if (attrs.hatching && "none" !== _normalizeEnum(attrs.hatching.direction)) {
            attrs = extend({}, attrs);
...
Renderer.prototype = {
    ...
    linearGradient: function(stops) {
        var gradient, id = getNextDefsSvgId(),
            that = this;
    ...
    pattern: function pattern(color, hatching, _id) {
        ...
        d = "right" === _normalizeEnum(hatching.direction) ? "M " + stepTo2 + " " + -stepTo2 + " L " + -stepTo2 + " " + stepTo2 + " M 0 " + step + " L " + step + " 0 M " + stepBy15 + " " + stepTo2 + " L " + stepTo2 + " " + stepBy15 : "M 0 0 L " + step + " " + step + " M " + -stepTo2 + " " + stepTo2 + " L " + stepTo2 + " " + stepBy15 + " M " + stepTo2 + " " + -stepTo2 + " L " + stepBy15 + " " + stepTo2;
