...
var dxChart = AdvancedChart.inherit({
    ...
    _applyPointMarkersAutoHiding: function() {
        ...
        that.panes.forEach(function(pane) {
            ...
            var argViewPortFilter = rangeCalculator.getViewPortFilter(argVisualRange || {});
            var points = [];
            ...
            series.forEach(function(s) {
                ...
                var valViewPortFilter = rangeCalculator.getViewPortFilter(valVisualRange || {});
            ...
            for (var i = 0; i < points.length; i++) {
                ...
                var distance = i + 1 > points.length - i - 1 ? i + 1 : points.length - i;
                if (_isDefined(curPoint.x) && _isDefined(curPoint.y)) {
                    for (var j = 1; j < distance; j++) {
                        var prevPoint = points[i - j];
                        var nextPoint = points[i + j];
                        var prev_x = _isDefined(prevPoint) ? prevPoint.x : null;
                        var prev_y = _isDefined(prevPoint) ? prevPoint.y : null;
                        var next_x = _isDefined(nextPoint) ? nextPoint.x : null;
                        var next_y = _isDefined(nextPoint) ? nextPoint.y : null;
            ...
            series.forEach(function(s) {
                ...
                if (s.autoHidePointMarkersEnabled() && ("discrete" === argAxisType || overloadedSeries[seriesIndex].pointsCount > argAxis.getTicksValues().majorTicksValues.length)) {
                    for (var index in overloadedSeries[seriesIndex]) {
                        ...
                        if (_i === seriesIndex) {
                            if (overloadedSeries[_i][_i] >= overloadedSeries[_i].pointsCount) {
                                s.autoHidePointMarkers = true;
                                break
                            }
                        } else {
                            if (overloadedSeries[seriesIndex].total >= 2 * overloadedSeries[seriesIndex].pointsCount) {
                                s.autoHidePointMarkers = true;
                                break
                            }
                        }
    ...
    _optionChanged: function(arg) {
        if (!this._optionChangedLocker) {
            if (arg.fullName.indexOf("visualRange") > 0) {
                ...
                var axis = this._valueAxes.filter(function(a) {
