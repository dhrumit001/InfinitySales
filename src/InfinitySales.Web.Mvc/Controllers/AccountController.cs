...
using InfinitySales.Identity;
using InfinitySales.MultiTenancy;
using InfinitySales.Sessions;
using InfinitySales.Web.Models.Account;
using InfinitySales.Web.Views.Shared.Components.TenantChange;
using InfinitySales.MultiTenancy.Dto;
using InfinitySales.UsersRegistration;
using Abp.Net.Mail;
...
{
    public class AccountController : InfinitySalesControllerBase
    {
        ...
        private async Task<AbpLoginResult<Tenant, User>> GetLoginResultAsync(string usernameOrEmailAddress, string password)
        {
            ...
            switch (loginResult.Result)
            {
                case AbpLoginResultType.Success:
                    return loginResult;
                default:
                    throw _abpLoginResultTypeHelper.CreateExceptionForFailedLoginAttempt(loginResult.Result, usernameOrEmailAddress, loginResult?.Tenant?.Name);
            }
        }
        ...
        public async Task<JsonResult> CheckDuplicateUsernameOrEmailAddress(string value)
        {
            using (CurrentUnitOfWork.DisableFilter("MayHaveTenant"))
            {
                return Json(await _userManager.CheckDuplicateUsernameOrEmailAddressAsync(null, value, value));
            }
        }
        ...
        {
            using (_unitOfWorkManager.Current.DisableFilter("MayHaveTenant"))
            {
                var user = await _userManager.FindByEmailAsync(forgotPasswordModel.UsernameOrEmailAddress);
                if (user == null || !(await _userManager.IsEmailConfirmedAsync(user)))
                {
                    // Don't reveal that the user does not exist or is not confirmed
                    return Json(new AjaxResponse { TargetUrl = Url.Action("ForgotPasswordConfirmation") });
                }
                ...
                _emailSender.Send(
                    to: forgotPasswordModel.UsernameOrEmailAddress,
        ...
        public async Task<JsonResult> ResetPassword(ResetPasswordViewModel model)
        {
            using (_unitOfWorkManager.Current.DisableFilter("MayHaveTenant"))
            {
                ...
                if (user == null)
                {
                    ...
                    return Json(new AjaxResponse { TargetUrl = Url.Action("ResetPasswordConfirmation") });
                }
        ...
        public virtual async Task<ActionResult> ExternalLoginCallback(string returnUrl, string remoteError = null)
        {
            ...
            var loginResult = await _logInManager.LoginAsync(externalLoginInfo);
